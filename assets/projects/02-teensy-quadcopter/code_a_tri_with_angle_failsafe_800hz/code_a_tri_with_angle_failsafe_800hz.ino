...
float KP_RateR = 0.4f, KI_RateR = 1.0f, KD_RateR = 0.0f;
float KP_RateP = 0.4f, KI_RateP = 3.0f, KD_RateP = 0.0f;
float KP_Y = 0.3f, KI_Y = 0.0f, KD_Y = 0.0000f;
...
void setup() {
  ...
  analogWriteFrequency(PIN_FR, 800);
  analogWriteFrequency(PIN_RR, 800);
  analogWriteFrequency(PIN_RL, 800);
  analogWriteFrequency(PIN_FL, 800);
...
// ---- FAILSAFE CHECK ----
  // check absolute values of Roll and Pitch
  if (abs(KalmanAngleRoll) > 20.0f || abs(KalmanAnglePitch) > 20.0f) {
    shutdownMotors();
    isFailsafeActive = true;
    Serial.println("!!! FAILSAFE TRIGGERED: EXCESSIVE TILT !!!");
  }
    ...
    float desY = 0;// (ReceiverValue[YAW_CH - 1] - 1500) * MAX_RATE_YAW / 2500.0f;
